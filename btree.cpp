...
void Getnext_btree(ifstream& inputfile,int no_of_buffer,int no_of_attr,double buffer_size )
{
...
void file_open(string ipfilename,int no_of_attr,int no_of_buffer,double buffer_size)
{
...
//    outputfile.open(opfilename.c_str());
    Getnext_btree(inputfile,no_of_buffer,no_of_attr,buffer_size);
    return ;
}
...
//        outputfile.open(opfilename.c_str(),ios::app);
        cout<<(stop_s-start_s)/double(CLOCKS_PER_SEC)<< " " << cnt_lines << endl;
//        outputfile.close();
    }
    return 0;
}